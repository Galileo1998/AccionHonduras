...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['csrf_token'])) {
    ...
    elseif ($_POST['action'] == 'save_lesson') {
        ...
        $title = trim($_POST['title']);
        $type = $_POST['content_type']; // Guardaremos 'builder' para las lecciones modulares
...
<body>
                ...
                <div class="module-body">
                    ...
                    <?php foreach($lecciones as $lec): ?>
                        <div class="lesson-item">
                            <div class="lesson-info" onclick="launchBlockBuilder(<?php echo $mod['id']; ?>, <?php echo $lec['id']; ?>, '<?php echo addslashes($lec['title']); ?>', '<?php echo addslashes($lec['content_html']); ?>')">
                                ...
                                <div>
                                    ...
                                    <span style="font-size:0.75rem; color:#94a3b8; margin-left:10px;">[Editor de Bloques] - Clic para modificar</span>
                                </div>
                            </div>
                    ...
                    <div class="lesson-form-box" id="block-builder-<?php echo $mod['id']; ?>">
                        ...
                        <form method="POST" id="form-sender-<?php echo $mod['id']; ?>" onsubmit="compileBlocksToJSON(<?php echo $mod['id']; ?>)">
                            ...
                            <input type="hidden" name="content_type" value="builder"> <input type="hidden" name="lesson_id" id="meta-lesson-id-<?php echo $mod['id']; ?>" value="0">
                            ...
                            <label style="font-size: 0.85rem; font-weight: bold; color: #475569;">Línea de Tiempo del Contenido (Arrastra u ordena tus componentes)</label>
    ...
    <script>
        ...
        function appendNewBlock(modId, type, value) {
            ...
            if (type === 'text') {
                placeholder = "Escribe el contenido de la lectura, admite formato libre...";
                inputField = `<textarea class="form-control block-input" rows="4" placeholder="${placeholder}">${value}</textarea>`;
            } else if (type === 'video') {
                ...
                placeholder = "Pega el enlace al archivo digital PDF (subido en tus medios)";
                inputField = `<input type="text" class="form-control block-input" value="${value}" placeholder="${placeholder}">`;
            }
        ...
        // SERIALIZADOR (COMPILADOR): Transforma los bloques visuales en un string JSON compacto antes de enviar al backend
        function compileBlocksToJSON(modId) {
            ...
            blocks.forEach(b => {
                ...
                const val = b.querySelector('.block-input').value.trim();
                dataPackage.push({ type: type, value: val });
            });
            
            document.getElementById('meta-json-output-<?php echo $mod['id']; ?>').value = JSON.stringify(dataPackage);
        }
    </script>
</body>
</html>