...
if (!empty($data->title) && !empty($data->slug)) {
    try {
        // Aseguramos que content_html tenga al menos un string vacío si no se envía
        $content_html = isset($data->content_html) ? $data->content_html : '';
        
        if (!empty($data->page_id) && is_numeric($data->page_id)) {
            // ACTUALIZAR
            $query = "UPDATE pages SET title = :title, slug = :slug, content_html = :content_html WHERE id = :id";
            $stmt = $db->prepare($query);
            $stmt->bindParam(':id', $data->page_id);
        } else {
            // CREAR NUEVA
            $query = "INSERT INTO pages (title, slug, status, content_html) VALUES (:title, :slug, 'published', :content_html)";
            $stmt = $db->prepare($query);
        }
